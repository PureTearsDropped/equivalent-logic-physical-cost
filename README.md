...
### 正しく丸める f32 exp を回路に(2026-09-06)

cuda 側の Ziv 高精度化を hardware 側へ移植した。64 語表・次数 6 の Estrin・60 ビット固定小数点・Ziv の出口、
fallback 無し。仕様モデルは MPFR 総当たりで範囲内の全 f32 入力 526,392,936 件が不一致 0・未決 0、ゲート版も全入力の
シミュレーションで不一致 0。配置配線後: 組合せ版 64.4 ns / 447k µm²、7 段 9.6 ns、**15 段 6.46 ns(155 MHz)/ 482k µm²**、
「全部標準フローに任せる」版は 6.99 ns / 710k µm²。OpenROAD のリサイザは効かず(1,137 個を太くして諦め)、配置で 10%、
乗算段の分割で 33%。**エネルギーは組合せ版 4,952 pJ/演算(グリッチ 80%)に対し 15 段版 318 pJ/演算(15 分の 1)**で、
この深さの算術ではパイプライン化はまず省エネの手段。詳細は `results/summary.md` §10、コードは `transcend/`。
