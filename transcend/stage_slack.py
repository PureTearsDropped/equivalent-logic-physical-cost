...
def stage_slack(design, period):
    net=build_exp(pipe=True); buffer_high_fanout(net)
...
"""
    ...
    for l in out.splitlines():
        ...
        st=stage_of.get(inst, 8 if (ep.startswith('y[') or ep=='decided') else None)
        if st is None: continue
        st=st-1 if st<8 else 7; worst[st]=min(worst[st],slack)
